    ...
    <>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:560px){.g2{grid-template-columns:1fr}.btn-s,.btn-xs{width:100%}}
        ...
        #hmap{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none;top:0;left:0}
      `}</style>
      ...
      <div className="container">
        ...
        <div className="panel">
          ...
          <div style={{marginBottom:'10px'}}>
            <label>都道府県</label>
            <select id="sPref" onChange={() => window.onPref()}><option value="">─ 選択 ─</option></select>
          </div>
          <div id="rowMulti" style={{display:'none'}}>
            <div className="rctrl">
              <button className="bxs" onClick={() => window.cityAll(true)}>全選択</button>
              <button className="bxs" onClick={() => window.cityAll(false)}>全解除</button>
        ...
        <div className="panel">
          ...
          <div className="rctrl">
            <button className="bxs" onClick={() => window.chipAll(true)}>全選択</button>
            <button className="bxs" onClick={() => window.chipAll(false)}>全解除</button>
        ...
        <div className="panel">
          ...
          <div className="cov">
            <strong>網羅的スキャン方式</strong>：エリアを格子状に分割し、各地点から半径内の場所を繰り返し検索します。
          </div>
          <div className="g2" style={{marginBottom:'12px'}}>
            <div>
              ...
              <select id="density">
                ...
                <option value="mid" selected>中（推奨）</option>
                <option value="high">高（網羅）</option>
              </select>
            </div>
            <div>
              <label>最低レビュー数</label>
              <select id="minRev">
                ...
                <option value="50" selected>50件以上</option>
                <option value="100">100件以上</option>
                <option value="500">500件以上</option>
              </select>
            </div>
          </div>
          <button className="btn-s" id="btnSearch" onClick={() => window.doSearch()}>🔍 スキャン開始</button>
          <div className="pbw" id="pbw">
            <div className="pb"><div className="pf" id="pf" style={{width:'0%'}} /></div>
          ...
          <div id="resSection" style={{display:'none',marginTop:'14px'}}>
            <div className="rh">
              ...
              <div style={{display:'flex',gap:'6px'}}>
                <button className="bxs" onClick={() => window.ckAll(true)}>全選択</button>
                <button className="bxs" onClick={() => window.ckAll(false)}>全解除</button>
              </div>
            </div>
            <div className="rl" id="resList" />
            <div className="ib">
              <span style={{fontSize:'.73rem',color:'var(--mut)'}} id="selInfo" />
              <button className="btn-s" style={{marginLeft:'auto',padding:'8px 16px',fontSize:'.82rem'}} onClick={() => window.importSel()}>
                ✅ 選択した場所を追加
              </button>
            </div>
          </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px',flexWrap:'wrap',gap:'8px'}}>
          <div className="sbar" style={{margin:0}}>
            <span>並び替え：</span>
            <button className="sbtn active" id="sR" onClick={() => window.setSort('r')}>レビュー数</button>
            <button className="sbtn" id="sRt" onClick={() => window.setSort('rt')}>評価</button>
            <button className="sbtn" id="sN" onClick={() => window.setSort('n')}>名前</button>
          </div>
          <button className="bxs" style={{color:'var(--danger)',borderColor:'#3a1515'}} onClick={() => window.clearAll()}>🗑 全削除</button>
        </div>
        <div className="tbar"><span id="totL">0件</span><span id="totR" /></div>

        <div className="vt">
          <button className="vb active" id="tabList" onClick={() => window.switchView('list')}>📋 リスト</button>
          <button className="vb" id="tabMap" onClick={() => window.switchView('map')}>🗺 マップ</button>
        </div>
...
function updCC(){document.getElementById('catCount').textContent=activeChips.size+' / '+CATS.length+' 選択中';}
...
CATS.forEach((c,i)=>{const b=document.createElement('button');b.className='chip';b.textContent=c.l;b.type='button';b.onclick=()=>{activeChips.has(i)?(activeChips.delete(i),b.classList.remove('on')):(activeChips.add(i),b.classList.add('on'));updCC();};chipsEl.appendChild(b);});
updCC();
const sPref=document.getElementById('sPref');
Object.keys(CITY_MAP).forEach(p=>{const o=document.createElement('option');o.value=p;o.textContent=p;sPref.appendChild(o);});
window.onPref=function(){
  const pref=sPref.value,cl=document.getElementById('cityList'),rw=document.getElementById('rowMulti');
  cl.innerHTML='';if(!pref){rw.style.display='none';return;}rw.style.display='block';
  (CITY_MAP[pref]||[]).forEach(c=>{const lbl=document.createElement('label');lbl.className='ccb';const cb=document.createElement('input');cb.type='checkbox';cb.value=c;cb.onchange=updCity;lbl.appendChild(cb);lbl.appendChild(document.createTextNode(c));cl.appendChild(lbl);});updCity();
};
function updCity(){const c=document.querySelectorAll('#cityList input:checked').length,t=document.querySelectorAll('#cityList input').length;document.getElementById('cityCount').textContent=c===0?'選択なし（都道府県全体）':c+' / '+t+' 選択中';}
window.cityAll=function(v){document.querySelectorAll('#cityList input[type=checkbox]').forEach(c=>c.checked=v);updCity();};
function getSelCities(){return[...document.querySelectorAll('#cityList input:checked')].map(c=>c.value);}
...
window.doSearch=async function(){
  const pref=sPref.value,selCities=getSelCities(),minRev=parseInt(document.getElementById('minRev').value)||0;
  ...
  setProg(0,'エリア座標を取得中...');srchRes=[];aborted=false;const seen=new Set();
  const ptSet=new Set();const gridPts=[];
  ...
  for(const cat of cats){if(aborted)break;for(const pt of gridPts){if(aborted)break;
    try{
      ...
      (data.places||[]).forEach(pl=>{
        if(seen.has(pl.id))return;seen.add(pl.id);const cnt=pl.userRatingCount||0;if(cnt<minRev)return;
  ...
  if(!srchRes.length){toast('結果が0件でした。エリアやカテゴリを確認してください','err');return;}
...
function renderRes(){
  ...
  document.getElementById('resList').innerHTML=srchRes.map((p,i)=>'<div class="ri"><input type="checkbox" id="rc'+i+'" checked onchange="updSel()"><label for="rc'+i+'" style="flex:1;min-width:0;cursor:pointer"><div class="rn">'+esc(p.name)+'</div><div class="rsb">'+(p.rating?'★'+p.rating+' · ':'')+' <span style="color:var(--accent)">'+p.count.toLocaleString()+'件</span> · '+esc(p.address)+'</div></label><div class="rc">'+fmtN(p.count)+'</div></div>').join('');
  updSel();
}
function updSel(){const t=srchRes.length,c=document.querySelectorAll('#resList input:checked').length;document.getElementById('selInfo').textContent=c+' / '+t+' 件選択中';}
window.ckAll=function(v){document.querySelectorAll('#resList input[type=checkbox]').forEach(c=>c.checked=v);updSel();};
window.importSel=function(){let added=0;document.querySelectorAll('#resList input[type=checkbox]').forEach((c,i)=>{if(c.checked&&srchRes[i]){pushPlace(srchRes[i]);added++;}});document.getElementById('resSection').style.display='none';srchRes=[];toast(added+'件追加しました','ok');};
...
function setP(p){aPref=p;aCity=null;render();}function setC(c){aCity=c;render();}
window.clearAll=function(){if(!confirm('全件削除しますか？'))return;places=[];aPref=null;aCity=null;render();};
window.switchView=function(v){curView=v;document.getElementById('tabList').classList.toggle('active',v==='list');document.getElementById('tabMap').classList.toggle('active',v==='map');document.getElementById('rankList').style.display=v==='list'?'flex':'none';document.getElementById('mapWrap').style.display=v==='map'?'flex':'none';if(v==='map')drawMap();};
const medals=['🥇','🥈','🥉'];
function render(){
  ...
  if(!prefs.length){fp.style.display='none';aPref=null;aCity=null;}
    ...
    document.getElementById('tagsPref').innerHTML=[null,...prefs].map(r=>{const cnt=r?places.filter(p=>p.pref===r).length:places.length;return'<button class="ftag'+(aPref===r?' ap':'')+'" onclick="setP('+(r?'\''+esc(r)+'\'':null)+')">'+(r||'すべて')+' '+cnt+'</button>';}).join('');
    const fc=document.getElementById('fcRow');
    if(aPref&&cities.length){fc.style.display='flex';document.getElementById('tagsCity').innerHTML=[null,...cities].map(c=>{const cnt=c?places.filter(p=>p.pref===aPref&&p.city===c).length:places.filter(p=>p.pref===aPref).length;return'<button class="ftag'+(aCity===c?' ac':'')+'" onclick="setC('+(c?'\''+esc(c)+'\'':null)+')">'+(c||'すべて')+' '+cnt+'</button>';}).join('');}else fc.style.display='none';
  }
  let filtered=[...places];
  if(aPref)filtered=filtered.filter(p=>p.pref===aPref);
  if(aCity)filtered=filtered.filter(p=>p.city===aCity);
  ...
  if(!sorted.length){list.innerHTML='<li class="empty">スキャンを実行してランキングを作成してください 🔍</li>';}
    ...
    return'<li class="ri2 '+(rank<=3?'r'+rank:'')+'" style="animation-delay:'+Math.min(i,.3)*.03+'s"><div class="bfl" style="width:'+pct+'%"></div><div class="rbg" style="color:'+gc+(rank===1?';text-shadow:0 0 12px rgba(245,200,66,.4)':'')+'">'+badge+'</div><div class="rif"><div class="rnm">'+esc(p.name)+'</div><div class="rmt"><span class="rst">'+ss+'</span>'+tp+tc+tt+'</div></div><div class="rct">'+fmtN(p.count)+'<small>reviews</small></div><button class="bdl" onclick="delPlace('+JSON.stringify(p.uid)+')">✕</button></li>';
  }).join('');
  switchView(curView);
}
...
function drawMap(){
  ...
  const wl=filtered.filter(p=>p.lat&&p.lng);
  if(!wl.length){wrap.innerHTML='<div class="me">マップ表示できる場所がありません</div>';return;}
  wrap.innerHTML='';wrap.style.display='block';
  const md=document.createElement('div');md.style.cssText='width:100%;height:420px';wrap.appendChild(md);
  const center={lat:wl.reduce((s,p)=>s+p.lat,0)/wl.length,lng:wl.reduce((s,p)=>s+p.lng,0)/wl.length};
  const map=new google.maps.Map(md,{center,zoom:13});
  const iw=new google.maps.InfoWindow();
  const sorted=getSorted(filtered);
  sorted.forEach((p,i)=>{if(!p.lat||!p.lng)return;const rank=i+1,col=rank===1?'#f5c842':rank===2?'#b0b8c1':rank===3?'#cd7f32':'#4285f4';
    const m=new google.maps.Marker({position:{lat:p.lat,lng:p.lng},map,label:{text:rank<=3?String(rank):'',color:'#fff',fontWeight:'bold',fontSize:'11px'},icon:{path:google.maps.SymbolPath.CIRCLE,scale:18,fillColor:col,fillOpacity:.9,strokeColor:'#fff',strokeWeight:2}});
    m.addListener('click',()=>{iw.setContent('<div style="font-family:sans-serif;padding:4px;max-width:220px"><div style="font-weight:700;font-size:13px;margin-bottom:3px">'+esc(p.name)+'</div><div style="font-size:12px;color:#444">'+(p.rating?'★'+p.rating+' · ':'')+p.count.toLocaleString()+'件</div><div style="font-size:11px;color:#777;margin-top:2px">'+esc(p.address)+'</div></div>');iw.open(map,m);});
  });
  const bounds=new google.maps.LatLngBounds();wl.forEach(p=>bounds.extend({lat:p.lat,lng:p.lng}));map.fitBounds(bounds);
}
let toastT;
function toast(msg,type){const el=document.getElementById('toast');el.textContent=msg;el.className='toast show'+(type?' '+type:'');clearTimeout(toastT);toastT=setTimeout(()=>el.classList.remove('show'),3000);}
render();switchView('list');
      `}} />
    </>
  );
}
